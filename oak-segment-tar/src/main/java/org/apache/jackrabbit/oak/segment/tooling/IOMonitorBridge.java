...
public class IOMonitorBridge implements org.apache.jackrabbit.oak.segment.file.tar.IOMonitor {
    private final Set<IOMonitor> delegates = newConcurrentHashSet();

    @Nonnull
    public Closeable addIOMonitor(@Nonnull IOMonitor ioMonitor) {
        delegates.add(checkNotNull(ioMonitor));
        return () -> delegates.remove(ioMonitor);
    }

    @Override
    public void beforeSegmentRead(File file, long msb, long lsb, int length) {
        delegates.forEach(ioMonitor ->
