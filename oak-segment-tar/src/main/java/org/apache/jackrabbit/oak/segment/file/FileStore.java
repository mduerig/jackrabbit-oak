    ...
    private class GarbageCollector {
        ...
        synchronized CompactionResult compact() {
            ...
            try {
                ...
                SegmentNodeState after = compact(null, before, compactor2, writer);
                if (after == null) {
                ...
                while (cycles < gcOptions.getRetryCount() &&
                    ...
                    after = compact(after, head, compactor2, writer);
                    if (after == null) {
                        gcListener.warn("TarMK GC #{}: compaction cancelled: {}.", GC_COUNT, cancel);
                        return compactionAborted(newGeneration);
                    }
                ...
                if (!success) {
                    ...
                    if (forceTimeout > 0) {
                        ...
                        after = forceCompact(after, compactor2, writer);
                        success = after != null;
                        if (success) {
        ...
        throws IOException {
            ...
            return writer.writeNode(builder.getNodeState(), uncompacted.getStableIdBytes());
        }
        ...
        private SegmentNodeState forceCompact(@Nonnull final SegmentNodeState base, @Nonnull final Compactor2 compactor2, SegmentWriter writer)
                ...
                public RecordId apply(RecordId headId) {
                    try {
                        ...
                        SegmentNodeState after = compact(
                               base, segmentReader.readNode(headId), compactor2, writer);
                        if (after == null) {
