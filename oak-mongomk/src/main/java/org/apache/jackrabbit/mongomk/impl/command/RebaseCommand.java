...
public class RebaseCommand extends BaseCommand<String> {
    ...
    private static NodeImpl rebaseNode(Node baseNode, Node fromNode, Node toNode, String path) {
        ...
        for (Entry<String, NodeState> removed : ourDelta.getRemovedChildNodes().entrySet()) {
            ...
            if (theirDelta.getRemovedChildNodes().containsKey(name)) {
                markConflict(stagedNode, "removeRemovedNode", name, ourState);
            }
            else if (theirDelta.getChangedChildNodes().containsKey(name)) {
                markConflict(stagedNode, "removeChangedNode", name, ourState);
            }
            else {
        ...
        for (Entry<String, NodeState> changed : ourDelta.getChangedChildNodes().entrySet()) {
            ...
            if (changedBase == null) {
                markConflict(stagedNode, "changeRemovedNode", name, ourState);
                continue;
            }
    ...
    private static void markConflict(NodeImpl parent, String conflictType, String name, String ourValue) {
        NodeImpl marker = addConflictMarker(parent, conflictType);
    ...
    private static NodeImpl addConflictMarker(NodeImpl parent, String name) {
        NodeImpl conflict = new NodeImpl(PathUtils.concat(parent.getPath(), ":conflict"));
        parent.addChildNodeEntry(conflict);

        NodeImpl marker = new NodeImpl(PathUtils.concat(conflict.getPath(), name));
        conflict.addChildNodeEntry(marker);
        return marker;
    }
